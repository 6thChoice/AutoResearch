...
class ContextCombiner:
    ...
    def __init__(self,
        ...
        # 运行时目录（planAgent 写入，优先级最高）
        self.runtime_global_dir = self.runtime_dir / ContextLevel.GLOBAL if self.runtime_dir else None
        self.runtime_project_dir = self.runtime_dir / ContextLevel.PROJECT if self.runtime_dir else None
        self.runtime_task_dir = self.runtime_dir / ContextLevel.TASK if self.runtime_dir else None

        # 项目级上下文也可以放在 workspace 中
        self.project_workspace_dir = self.workspace_dir / ".context"

    def _collect_from_dirs(self, *dirs: Optional[Path]) -> List[Path]:
        """从多个目录收集 md 文件，后面的目录优先级更高（同名覆盖）"""
        seen: dict[str, Path] = {}
        for d in dirs:
            if d and d.exists():
    ...
    def get_context_files(self, level: str, task_id: Optional[str] = None) -> List[Path]:
        """获取指定层级的所有上下文文件（运行时 > workspace > 模板）"""
        if level == ContextLevel.GLOBAL:
            return self._collect_from_dirs(self.global_dir, self.runtime_global_dir)
        elif level == ContextLevel.PROJECT:
            return self._collect_from_dirs(
                self.project_dir,
                self.project_workspace_dir if self.project_workspace_dir.exists() else None,
                self.runtime_project_dir,
            )
        elif level == ContextLevel.TASK:
            template_dir = (self.task_dir / task_id) if task_id else self.task_dir
            return self._collect_from_dirs(template_dir, self.runtime_task_dir)
        return []
...
示例:
  ...
  python3 combine_context.py --task-id task-001 --inject task.md --output combined.md
    ...
    parser.add_argument("--runtime-dir", default=None,
                        help="运行时上下文目录（planAgent 写入，优先级最高）")
